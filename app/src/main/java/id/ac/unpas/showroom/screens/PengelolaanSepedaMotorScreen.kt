...
@Composable
fun PengelolaanSepedaMotorScreen(navController : NavHostController, snackbarHostState: SnackbarHostState, modifier: Modifier = Modifier) {
    ...
    Column(modifier = Modifier.fillMaxWidth()) {
        Button(onClick = {
            navController.navigate("tambah-pencatatan-sepeda-motor")
        }) {
            Text(text = "Tambah")
        }
            ...
            items(items = items, itemContent = { item ->
                Row(modifier = Modifier
                    ...
                    Column(modifier = Modifier.weight(3f)) {
                        ...
                        Text(text = item.kapasitas, fontSize = 16.sp,
                            fontWeight = FontWeight.Bold)
                    }
                }
                Row(Modifier.padding(top = 0.dp, start = 15.dp, end = 15.dp, bottom = 15.dp).fillMaxWidth().clickable {
                    navController.navigate("edit-pengelolaan-sepeda-motor/${item.id}")
                }) {
                    Column(modifier = Modifier.weight(3f)) {
                        Text(text = "Tanggal Rilis", fontSize = 14.sp)
                        Text(text = item.tanggal_rilis, fontSize = 16.sp, fontWeight =
                        FontWeight.Bold)
                        Column(modifier = Modifier.fillMaxWidth()) {
